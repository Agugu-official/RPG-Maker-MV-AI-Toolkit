...
export class RPGMakerWriter {
  ...
  listPlugins(): Array<{ name: string; status: boolean; description: string; parameters: Record<string, unknown> }> {
    ...
    try {
      ...
      return this.extractPluginsArray(content).map((p) => ({
        name: p.name,
  ...
  removePluginFromRegistry(pluginName: string): void {
    ...
    if (!fs.existsSync(pluginsJsPath)) return;
    const content = fs.readFileSync(pluginsJsPath, "utf-8");
    const plugins = this.extractPluginsArray(content).filter((p) => p.name !== pluginName);
  ...
  deleteMap(mapId: number): void {
    const mapFilename = `Map${String(mapId).padStart(3, "0")}.json`;
    const mapFilePath = path.join(this.dataPath, mapFilename);
    if (!fs.existsSync(mapFilePath)) {
      throw new Error(`Map file not found: ${mapFilename}`);
    }
    this.createBackupFile(mapFilename);
    ...
    if (fs.existsSync(mapInfosPath)) {
      try {
        ...
        if (mapId < mapInfos.length) {
          mapInfos[mapId] = null;
          this.writeJsonFile("MapInfos.json", mapInfos);
        }
      } catch { /* best-effort */ }
    }
    try { this.refreshVersionId(); } catch { /* best-effort */ }
  }
